use std::fmt::Display;
use std::fmt::Debug;
use field::Pow;
use field::Field;
use field::extended_euclid;
use std::convert::From;
use std::ops::{Add, Div, Mul, Sub};
use std::fmt;
use num::{Num, Integer, One, Zero};
use num::bigint::{BigInt, BigUint, Sign, ToBigInt};
use serde::{Serialize, Serializer};
use serde::de::{Deserialize, Deserializer, Visitor};

lazy_static! {
    static ref P: BigInt = BigInt::parse_bytes(b"73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001", 16).unwrap();
}

#[derive(PartialEq, PartialOrd, Clone, Eq, Ord)]
pub struct Bls12Field {
    value: BigInt,
}

impl Field for Bls12Field {
    fn into_byte_vector(&self) -> Vec<u8> {
        match self.value.to_biguint() {
            Option::Some(val) => val.to_bytes_le(),
            Option::None => panic!("Should never happen."),
        }
    }

    fn from_byte_vector(bytes: Vec<u8>) -> Self {
        let uval = BigUint::from_bytes_le(bytes.as_slice());
        Bls12Field{value: BigInt::from_biguint(Sign::Plus, uval)}
    }

    fn to_dec_string(&self) -> String {
        self.value.to_str_radix(10)
    }

    fn from_dec_string(val: String) -> Self {
        Bls12Field{value: BigInt::from_str_radix(val.as_str(), 10).unwrap()}
    }

    fn inverse_mul(&self) -> Bls12Field {
        let (b, s, _) = extended_euclid(&self.value, &*P);
        assert_eq!(b, BigInt::one());
        Bls12Field {
            value: &s - s.div_floor(&*P) * &*P,
        }
    }
    fn min_value() -> Bls12Field {
        Bls12Field {
            value: ToBigInt::to_bigint(&0).unwrap(),
        }
    }
    fn max_value() -> Bls12Field {
        Bls12Field {
            value: &*P - ToBigInt::to_bigint(&1).unwrap(),
        }
    }
    fn get_required_bits() -> usize {
        (*P).bits()
    }
}

impl Display for Bls12Field {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.value.to_str_radix(10))
    }
}

impl Debug for Bls12Field {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.value.to_str_radix(10))
    }
}

impl From<i32> for Bls12Field {
    fn from(num: i32) -> Self {
        let x = ToBigInt::to_bigint(&num).unwrap();
        Bls12Field {
            value: &x - x.div_floor(&*P) * &*P,
        }
    }
}

impl From<u32> for Bls12Field {
    fn from(num: u32) -> Self {
        let x = ToBigInt::to_bigint(&num).unwrap();
        Bls12Field {
            value: &x - x.div_floor(&*P) * &*P,
        }
    }
}

impl From<usize> for Bls12Field {
    fn from(num: usize) -> Self {
        let x = ToBigInt::to_bigint(&num).unwrap();
        Bls12Field {
            value: &x - x.div_floor(&*P) * &*P,
        }
    }
}

impl<'a> From<&'a str> for Bls12Field {
    fn from(s: &'a str) -> Self {
        let x = match BigInt::parse_bytes(s.as_bytes(), 10) {
            Some(x) => x,
            None => panic!("Could not parse {:?} to BigInt!", &s),
        };
        Bls12Field {
            value: &x - x.div_floor(&*P) * &*P,
        }
    }
}

impl Zero for Bls12Field {
    fn zero() -> Bls12Field {
        Bls12Field {
            value: ToBigInt::to_bigint(&0).unwrap(),
        }
    }
    fn is_zero(&self) -> bool {
        self.value == ToBigInt::to_bigint(&0).unwrap()
    }
}

impl One for Bls12Field {
    fn one() -> Bls12Field {
        Bls12Field {
            value: ToBigInt::to_bigint(&1).unwrap(),
        }
    }
}

impl Add<Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn add(self, other: Bls12Field) -> Bls12Field {
        Bls12Field {
            value: (self.value + other.value) % &*P,
        }
    }
}

impl<'a> Add<&'a Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn add(self, other: &Bls12Field) -> Bls12Field {
        Bls12Field {
            value: (self.value + other.value.clone()) % &*P,
        }
    }
}

impl Sub<Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn sub(self, other: Bls12Field) -> Bls12Field {
        let x = self.value - other.value;
        Bls12Field {
            value: &x - x.div_floor(&*P) * &*P,
        }
    }
}

impl<'a> Sub<&'a Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn sub(self, other: &Bls12Field) -> Bls12Field {
        let x = self.value - other.value.clone();
        Bls12Field {
            value: &x - x.div_floor(&*P) * &*P,
        }
    }
}

impl Mul<Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn mul(self, other: Bls12Field) -> Bls12Field {
        Bls12Field {
            value: (self.value * other.value) % &*P,
        }
    }
}

impl<'a> Mul<&'a Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn mul(self, other: &Bls12Field) -> Bls12Field {
        Bls12Field {
            value: (self.value * other.value.clone()) % &*P,
        }
    }
}

impl Div<Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn div(self, other: Bls12Field) -> Bls12Field {
        self * other.inverse_mul()
    }
}

impl<'a> Div<&'a Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn div(self, other: &Bls12Field) -> Bls12Field {
        self / other.clone()
    }
}

impl Pow<usize> for Bls12Field {
    type Output = Bls12Field;

    fn pow(self, exp: usize) -> Bls12Field {
        let mut res = Bls12Field::from(1);
        for _ in 0..exp {
            res = res * &self;
        }
        res
    }
}

impl Pow<Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn pow(self, exp: Bls12Field) -> Bls12Field {
        let mut res = Bls12Field::one();
        let mut current = Bls12Field::zero();
        loop {
            if current >= exp {
                return res;
            }
            res = res * &self;
            current = current + Bls12Field::one();
        }
    }
}

impl<'a> Pow<&'a Bls12Field> for Bls12Field {
    type Output = Bls12Field;

    fn pow(self, exp: &'a Bls12Field) -> Bls12Field {
        let mut res = Bls12Field::one();
        let mut current = Bls12Field::zero();
        loop {
            if &current >= exp {
                return res;
            }
            res = res * &self;
            current = current + Bls12Field::one();
        }
    }
}

// custom serde serialization
impl Serialize for Bls12Field {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
        where S: Serializer
    {
        // serializer.serialize_bytes(&(*self.value.to_biguint().to_bytes_le().as_slice()))
        serializer.serialize_bytes(&(*self.into_byte_vector().as_slice()))
    }
}

// custom serde deserialization

struct FieldPrimeVisitor;

impl FieldPrimeVisitor {
    fn new() -> Self {
        FieldPrimeVisitor{}
    }
}

impl<'de> Visitor<'de> for FieldPrimeVisitor {
    type Value = Bls12Field;

    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        formatter.write_str("struct Bls12Field")
    }

    fn visit_bytes<E>(self, v: &[u8]) -> Result<Self::Value, E> {
        let val = BigUint::from_bytes_le(v).to_bigint().unwrap();
        Ok(Bls12Field{value: val})
    }
}

impl<'de> Deserialize<'de> for Bls12Field {

    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
        where D: Deserializer<'de>
    {
        deserializer.deserialize_bytes(FieldPrimeVisitor::new())
    }
}

#[cfg(test)]
mod field_prime {
    use super::*;
    use bincode::{serialize, deserialize , Infinite};

    #[test]
    fn positive_number() {
        assert_eq!(
            "1234245612".parse::<BigInt>().unwrap(),
            Bls12Field::from("1234245612").value
        );
    }

    #[test]
    fn negative_number() {
        assert_eq!(
            P.checked_sub(&"12".parse::<BigInt>().unwrap()).unwrap(),
            Bls12Field::from("-12").value
        );
    }

    #[test]
    fn addition() {
        assert_eq!(
            "65484493".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") + Bls12Field::from("68135")).value
        );
        assert_eq!(
            "65484493".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") + &Bls12Field::from("68135")).value
        );
    }

    #[test]
    fn addition_negative_small() {
        assert_eq!(
            "3".parse::<BigInt>().unwrap(),
            (Bls12Field::from("5") + Bls12Field::from("-2")).value
        );
        assert_eq!(
            "3".parse::<BigInt>().unwrap(),
            (Bls12Field::from("5") + &Bls12Field::from("-2")).value
        );
    }

    #[test]
    fn addition_negative() {
        assert_eq!(
            "65348223".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") + Bls12Field::from("-68135")).value
        );
        assert_eq!(
            "65348223".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") + &Bls12Field::from("-68135")).value
        );
    }

    #[test]
    fn subtraction() {
        assert_eq!(
            "65348223".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") - Bls12Field::from("68135")).value
        );
        assert_eq!(
            "65348223".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") - &Bls12Field::from("68135")).value
        );
    }

    #[test]
    fn subtraction_negative() {
        assert_eq!(
            "65484493".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") - Bls12Field::from("-68135")).value
        );
        assert_eq!(
            "65484493".parse::<BigInt>().unwrap(),
            (Bls12Field::from("65416358") - &Bls12Field::from("-68135")).value
        );
    }

    #[ignore]
    #[test]
    fn subtraction_overflow() {
        assert_eq!(
            "21888242871839275222246405745257275088548364400416034343698204186575743147394"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from("68135") - Bls12Field::from("65416358")).value
        );
        assert_eq!(
            "21888242871839275222246405745257275088548364400416034343698204186575743147394"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from("68135") - &Bls12Field::from("65416358")).value
        );
    }

    #[test]
    fn multiplication() {
        assert_eq!(
            "13472".parse::<BigInt>().unwrap(),
            (Bls12Field::from("32") * Bls12Field::from("421")).value
        );
        assert_eq!(
            "13472".parse::<BigInt>().unwrap(),
            (Bls12Field::from("32") * &Bls12Field::from("421")).value
        );
    }

    #[ignore]
    #[test]
    fn multiplication_negative() {
        assert_eq!(
            "21888242871839275222246405745257275088548364400416034343698204186575808014369"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from("54") * Bls12Field::from("-8912")).value
        );
        assert_eq!(
            "21888242871839275222246405745257275088548364400416034343698204186575808014369"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from("54") * &Bls12Field::from("-8912")).value
        );
    }

    #[test]
    fn multiplication_two_negative() {
        assert_eq!(
            "648".parse::<BigInt>().unwrap(),
            (Bls12Field::from("-54") * Bls12Field::from("-12")).value
        );
        assert_eq!(
            "648".parse::<BigInt>().unwrap(),
            (Bls12Field::from("-54") * &Bls12Field::from("-12")).value
        );
    }

    #[ignore]
    #[test]
    fn multiplication_overflow() {
        assert_eq!(
            "6042471409729479866150380306128222617399890671095126975526159292198160466142"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from(
                "21888242871839225222246405785257275088694311157297823662689037894645225727"
            ) *
                Bls12Field::from("218882428715392752222464057432572755886923"))
                .value
        );
        assert_eq!(
            "6042471409729479866150380306128222617399890671095126975526159292198160466142"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from(
                "21888242871839225222246405785257275088694311157297823662689037894645225727"
            ) *
                &Bls12Field::from("218882428715392752222464057432572755886923"))
                .value
        );
    }

    #[test]
    fn division() {
        assert_eq!(
            Bls12Field::from(4),
            Bls12Field::from(48) / Bls12Field::from(12)
        );
        assert_eq!(
            Bls12Field::from(4),
            Bls12Field::from(48) / &Bls12Field::from(12)
        );
    }

    #[test]
    fn division_negative() {
        let res = Bls12Field::from(-54) / Bls12Field::from(12);
        assert_eq!(Bls12Field::from(-54), Bls12Field::from(12) * res);
    }

    #[test]
    fn division_two_negative() {
        let res = Bls12Field::from(-12) / Bls12Field::from(-85);
        assert_eq!(Bls12Field::from(-12), Bls12Field::from(-85) * res);
    }

    #[test]
    fn pow_small() {
        assert_eq!(
            "8".parse::<BigInt>().unwrap(),
            (Bls12Field::from("2").pow(Bls12Field::from("3"))).value
        );
        assert_eq!(
            "8".parse::<BigInt>().unwrap(),
            (Bls12Field::from("2").pow(&Bls12Field::from("3"))).value
        );
    }

    #[test]
    fn pow_usize() {
        assert_eq!(
            "614787626176508399616".parse::<BigInt>().unwrap(),
            (Bls12Field::from("54").pow(12)).value
        );
    }

    #[test]
    fn pow() {
        assert_eq!(
            "614787626176508399616".parse::<BigInt>().unwrap(),
            (Bls12Field::from("54").pow(Bls12Field::from("12"))).value
        );
        assert_eq!(
            "614787626176508399616".parse::<BigInt>().unwrap(),
            (Bls12Field::from("54").pow(&Bls12Field::from("12"))).value
        );
    }

    #[ignore]
    #[test]
    fn pow_negative() {
        assert_eq!(
            "21888242871839275222246405745257275088548364400416034343686819230535502784513"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from("-54").pow(Bls12Field::from("11"))).value
        );
        assert_eq!(
            "21888242871839275222246405745257275088548364400416034343686819230535502784513"
                .parse::<BigInt>()
                .unwrap(),
            (Bls12Field::from("-54").pow(&Bls12Field::from("11"))).value
        );
    }

    #[test]
    fn serde_ser_deser() {
        let serialized = &serialize(&Bls12Field::from("11"), Infinite).unwrap();
        let deserialized = deserialize(serialized).unwrap();
        assert_eq!(Bls12Field::from("11"), deserialized);
    }

    #[test]
    fn bytes_ser_deser() {
        let fp = Bls12Field::from("101");
        let bv = fp.into_byte_vector();
        assert_eq!(fp, Bls12Field::from_byte_vector(bv));
    }

    #[test]
    fn dec_string_ser_deser() {
        let fp = Bls12Field::from("101");
        let bv = fp.to_dec_string();
        assert_eq!(fp, Bls12Field::from_dec_string(bv));
    }
}

#[test]
fn bigint_assertions() {
    let x = BigInt::parse_bytes(b"65", 10).unwrap();
    assert_eq!(&x + &x, BigInt::parse_bytes(b"130", 10).unwrap());
    assert_eq!(
        "1".parse::<BigInt>().unwrap(),
        "3".parse::<BigInt>()
            .unwrap()
            .div_floor(&"2".parse::<BigInt>().unwrap())
    );
    assert_eq!(
        "-2".parse::<BigInt>().unwrap(),
        "-3".parse::<BigInt>()
            .unwrap()
            .div_floor(&"2".parse::<BigInt>().unwrap())
    );
}

#[ignore]
#[test]
fn test_extended_euclid() {
    assert_eq!(
        (
            ToBigInt::to_bigint(&1).unwrap(),
            ToBigInt::to_bigint(&-9).unwrap(),
            ToBigInt::to_bigint(&47).unwrap()
        ),
        extended_euclid(
            &ToBigInt::to_bigint(&120).unwrap(),
            &ToBigInt::to_bigint(&23).unwrap()
        )
    );
    assert_eq!(
        (
            ToBigInt::to_bigint(&2).unwrap(),
            ToBigInt::to_bigint(&2).unwrap(),
            ToBigInt::to_bigint(&-11).unwrap()
        ),
        extended_euclid(
            &ToBigInt::to_bigint(&122).unwrap(),
            &ToBigInt::to_bigint(&22).unwrap()
        )
    );
    assert_eq!(
        (
            ToBigInt::to_bigint(&2).unwrap(),
            ToBigInt::to_bigint(&-9).unwrap(),
            ToBigInt::to_bigint(&47).unwrap()
        ),
        extended_euclid(
            &ToBigInt::to_bigint(&240).unwrap(),
            &ToBigInt::to_bigint(&46).unwrap()
        )
    );
    let (b, s, _) = extended_euclid(&ToBigInt::to_bigint(&253).unwrap(), &*P);
    assert_eq!(b, BigInt::one());
    let s_field = Bls12Field {
        value: &s - s.div_floor(&*P) * &*P,
    };
    assert_eq!(
        Bls12Field::from(
            "12717674712096337777352654721552646000065650461901806515903699665717959876900"
        ),
        s_field
    );
}